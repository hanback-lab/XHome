...
## 시간에 따른 전등 제어 
조명은 사람에 따라 다르지만, 아침시간에 조명을 끄고, 저녁시간에 조명을 켜는등 일반적인 사용 패턴이 존재합니다. 현재 시스템에 시간에 따라서 제어를 구현해 보겠습니다. time 모듈의 localtime() 은 시스템의 현재 시간을 날짜와 시, 분, 초 등의 정보를 확인할 수 있습니다. 이 정보를 내초마다 확인하여 지정된 시간에 따라 제어해보도록하겠습니다. 
...
![seccnario ui](res/secnario.png)

Designer를 통해 구현하며, 이미 구현되어 파이썬으로 변환된 코드의 내용을 참조하고싶다면 아래 내용을 참조하시기 바랍니다. 
...
################################################################################
## Form generated from reading UI file 'scenraio.ui'
##
...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.scenraioComboBox = QComboBox(Form)
        self.scenraioComboBox.addItem("")
        self.scenraioComboBox.addItem("")
        self.scenraioComboBox.addItem("")
        self.scenraioComboBox.setObjectName(u"scenraioComboBox")
        self.scenraioComboBox.setGeometry(QRect(50, 390, 381, 71))
        self.scenraioComboBox.setFont(font)
        self.scenraioActiveBtn = QPushButton(Form)
        self.scenraioActiveBtn.setObjectName(u"scenraioActiveBtn")
        self.scenraioActiveBtn.setGeometry(QRect(510, 390, 181, 71))
        font3 = QFont()
        font3.setPointSize(20)
        self.scenraioActiveBtn.setFont(font3)
        self.scenraioLbl = QLabel(Form)
        self.scenraioLbl.setObjectName(u"scenraioLbl")
        self.scenraioLbl.setGeometry(QRect(20, 320, 201, 71))
        self.scenraioLbl.setFont(font1)
        self.scenraioLbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def retranslateUi(self, Form):
        ...
        self.scenraioComboBox.setItemText(0, QCoreApplication.translate("Form", u"Present", None))
        self.scenraioComboBox.setItemText(1, QCoreApplication.translate("Form", u"Sleeping", None))
        self.scenraioComboBox.setItemText(2, QCoreApplication.translate("Form", u"Outing", None))

        self.scenraioActiveBtn.setText(QCoreApplication.translate("Form", u"Active", None))
        self.scenraioLbl.setText(QCoreApplication.translate("Form", u"Scenraio", None))
```
</details>

### 동작 코드 구현 
크게 정해진 시나리오에 따른 제어, 수동 제어의 기능을 구현합니다. 시나리오 동작에는 동작이 수행될 때 Slack을 통해 메시지를 전달하여 현재 동작 모드에 대한 메시지를 전달하여 원격으로 확인할 수 있도록 합니다. 전체 기능을 정리하면 다음과 같습니다. 
...
from scenraio_ui import Ui_Form
import sys

class Scenraio(QWidget):
    def __init__(self, parent=None):
        ...
        self.ui.scenraioActiveBtn.clicked.connect(self.scenraio_actvation_btn_pressed)

    def on_btn_pressed(self):
        if self.flagScenraio:
            return
    ...
    def mood_set_btn_pressed(self):
        if self.flagScenraio == "Sleeping":
            return 
        
        self.moodlamp.setColor(self.ui.redSpinBox.value(), self.ui.greenSpinBox.value(), self.ui.blueSpinBox.value())

    def scenraio_actvation_btn_pressed(self):
        if not self.flagScenraio:
            self.ui.scenraioComboBox.setDisabled(True)
            self.ui.scenraioActiveBtn.setText("Deactivate")

            if self.ui.scenraioComboBox.currentText() == "Present":
                self.flagScenraio = "Present"
                self.lamp.on()
            elif self.ui.scenraioComboBox.currentText() == "Sleeping":
                self.flagScenraio = "Sleeping"
                self.lamp.off()
                self.moodlamp.setColor(127,0,255)
            elif self.ui.scenraioComboBox.currentText() == "Outing":
                self.flagScenraio = "Outing"
                self.lamp.off()
                self.moodlamp.setColor(0,0,0)
                self.doorlock.close()
                self.curtain.close()
            response = self.slack_client.chat_postMessage(channel='#bot-test', text=f"{self.flagScenraio} is activated.")
        else:
            self.ui.scenraioComboBox.setDisabled(False)
            self.ui.scenraioActiveBtn.setText("Activate")
            self.flagScenraio = False
            response = self.slack_client.chat_postMessage(channel='#bot-test', text=f"{self.flagScenraio} is deactivated.")
